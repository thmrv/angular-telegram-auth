...
export class BackendService {
  ...
  private readonly LOGOUT_ENDPOINT = environment.logoutEndpoint || '/api/auth/logout';
  ...
  /**
   * Logout: send a POST request to /api/auth/logout with CSRF token
   */
  logout(): Observable<LogoutResponse> {
    return this.ensureSessionInitialized().pipe(
      switchMap(() => {
        const url = this.getFullUrl(this.LOGOUT_ENDPOINT);
        const csrfToken = this.csrfTokenSubject.value;
        ...
        return this.http.post<LogoutResponse>(url, {}, { headers })
          .pipe(
            timeout(10000),
            map((response) => {
              console.log('Logout response:', response);
              // After successful logout, clear the session state
              this.isInitialized = false;
              this.csrfTokenSubject.next(null);
              this.sessionIdSubject.next(null);
              this.lastAuthProvider = null;
            ...
            catchError((error) => {
              // Even if the logout fails on the backend, clear local state
              // to prevent the user from being stuck in a half-logged-in state
              console.warn('Logout error, clearing local session anyway:', error);
              this.isInitialized = false;
              this.csrfTokenSubject.next(null);
              this.sessionIdSubject.next(null);
              this.lastAuthProvider = null;
