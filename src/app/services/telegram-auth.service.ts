...
export class TelegramAuthService {
  ...
  private async getEmbedWidgetUrl(): Promise<any> {
    ...
    /*const url = `${this.AUTH_URI}?` +
      ...
      `&origin=${origin}`;*/
    const url = `https://oauth.telegram.org/auth?response_type=post_message&client_id=${this.BOT_ID}&redirect_uri=${this.REDIRECT_URI}&scope=openid profile telegram%3Abot_access&origin=${origin}`;
    return url;
  }
